...
class DynamicAdapt {
  ...
  init() {
    ...
    this.nodes.forEach((node) => {
      const data2 = node.dataset.flsDynamic.trim();
...
function initBlog(data2, startItem2, endItem2) {
  const dataPart = data2.items.slice(startItem2, endItem2);
...
function viewMore() {
  ...
  if (viewMoreBtn) viewMoreBtn.hidden = false;
}
function documentActions(e) {
  const viewMoreBtn = e.target.closest(".blog__view-more");
  ...
  if (!expanded) {
    ...
    initBlog(data, startItem, endItem);
    expanded = true;
    viewMoreBtn.textContent = "Hidden";
  } else {
    const allItems = document.querySelectorAll(".item-blog");
  ...
  viewMoreBtn.classList.toggle("rotated", expanded);
  e.preventDefault();
}
